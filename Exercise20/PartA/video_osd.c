...
//    }
    ...
    for(j=osdInfo.yres/2; j<osdInfo.yres; j++)
	for(i=osdInfo.xres/2; i<osdInfo.xres; i++) {
	    (*osdDisplayByRef)[i + j*osdInfo.xres] = 
		(trans<<24) | 0x00ffff00;	// AARRGGBB
	}
...
int video_osd_circframe( unsigned int * osdDisplay, unsigned int  fillval )
{
    ...
    x_scale = (float) (osdInfo.xres >> 2)*3;
    y_scale = (float) (osdInfo.yres >> 2)*3;

    DBG( "Entering video_osd_circframe for-loop\n" );

    for( j = 0; j < osdInfo.yres; j++ ) {
        y = ( float )j - y_scale;
        for( i = 0; i < osdInfo.xres; i++ ) {
            x = ( float )i - x_scale;
	    if (((x*x /(x_scale*x_scale)) + (y*y/(y_scale*y_scale))) > 0.9) {
                osdDisplay[i+(osdInfo.xres/2) + j*osdInfo.xres] = fillval;
            }
        }
    }
